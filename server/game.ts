import { Command, Direction } from "../common/types";
...
export class Game {
    ...
    playerCommand(player: PlayerSocket, command: string) {
        ...
        if (gamePlayer && gamePlayer.active && !gamePlayer.moved) {
            switch(command) {
                ...
                case 'left':
                    ...
                    break;
                case 'fire_wave':
                    let wave = getWaveElements('fire', player);
                    this.elements = this.elements.concat(wave);
                    break;
                case 'fire_field': 
                    let field = getFieldElements('fire', player);
                    this.elements = this.elements.concat(field);
                    break;
            }
        }
    }
